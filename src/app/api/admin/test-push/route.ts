...
export async function POST(req: Request) {
    try {
        ...
        const results: { endpoint: string; ok: boolean; error?: string; statusCode?: number; deliveryId?: string }[] = [];
        pushLog("4. Enviando push a cada suscripción...");

        for (const sub of subs) {
            const endpointShort = sub.endpoint.slice(0, 60) + "...";
            const deliveryId = randomUUID();
            const message = JSON.stringify({ ...payload, deliveryId });
            try {
                const keys = {
                ...
                results.push({ endpoint: endpointShort, ok: true, deliveryId });
                ...
                await PushDeliveryLog.create({
                    ...
                    subscriptionId: String(sub._id),
                    status: "failed",
                    errorMessage: `${msg} (status: ${status})`,
                    sentAt: new Date(),
                    errorAt: new Date(),
                });
            }
